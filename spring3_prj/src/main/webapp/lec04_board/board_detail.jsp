...
<c:forEach items="${KEY_REPLYLIST}" var="rvo">
	...
	<c:if test="${not empty rvo.rseq}">
		<form method="post" action="${pageContext.request.contextPath}/reply_update?seq=${rvo.seq}&rseq=${rvo.rseq}">
			<tr><td><font color=red>
			<a href="${pageContext.request.contextPath}/reply_delete?seq=${rvo.seq}&rseq=${rvo.rseq}">[X]</a>
...
<!-- --------------------------------------- 댓글등록 ------------------------------- -->
<br>
<form id="replyForm" method="post" action="${pageContext.request.contextPath}/reply_insert">
<input type=hidden name="seq" value="${KEY_BOARDVO.seq}">
<table width=100% border=1>
<tr>
	<td>
