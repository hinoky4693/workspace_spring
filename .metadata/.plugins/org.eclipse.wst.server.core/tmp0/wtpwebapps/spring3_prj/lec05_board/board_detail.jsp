...
<tr>
      <th>내용</th>
      <td><textarea name="contents" cols="80" rows="6">${KEY_BOARDVO.title}</textarea></td>
</tr>
<tr>
...
<c:forEach items="${KEY_BOARDVO.replies}" var="rvo">
		...
		<form method="post" action="${pageContext.request.contextPath}/reply_update?seq=${KEY_BOARDVO.seq}&rseq=${rvo.rseq}">
			<tr><td><font color=red>
			<a href="${pageContext.request.contextPath}/reply_delete?seq=${KEY_BOARDVO.seq}&rseq=${rvo.rseq}">[X]</a>
			<input name="reply" type="text" value="${rvo.reply}">
...
<tr>
	<td>
		...
		<input type="button" value="댓글등록" id="replyInsertButton">
	</td>
</tr>
</table>
</form>
			...
			$.ajax({
		        ...
		        data 		: "seq="+${KEY_BOARDVO.seq},
		  		...
		  		success     : function(myval){
		  										var htmlStr = "<table border=1 width=100%>";
		  										$.map( myval, function( MYval, MYidx ) {
		  											htmlStr += "<tr><td><input type='button' id ="+MYval.rseq+" data-rseq="+MYval.rseq+" class='replyDeleteButton' value='[X]'>" + MYval.reply + "</td></tr>"
		  										});
		  										htmlStr += "</table>";
		  										$("#replyListDiv").empty();
		  										$("#replyListDiv").html(htmlStr);
